...
while ~strcmp(key, 'q') && path_ind <= size(path, 2)
    switch alg
        case 'bug 0'
            ...
            u_list = bug0(x_est, path(:, path_ind), z_lidar, ang_lidar, col_buffer, max_linear, max_angular);
        case 'potential field'
            u_list = potential_field(x_est, path(:, path_ind), new_points, max_linear, max_angular,...
